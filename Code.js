...
function logSecondaryViewInteraction(name, isoTimestamp, runningTotal) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  ...
  if (sheet.getLastRow() === 0) {
    sheet.appendRow(['Interaction', 'Time', 'Running Total']);
  }

  var tz = Session.getScriptTimeZone();
  var ts = isoTimestamp ? new Date(isoTimestamp) : new Date();
  if (isNaN(ts.getTime())) {
    ts = new Date();
  }
  var formatted = Utilities.formatDate(ts, tz, 'MM/dd/yyyy HH:mm:ss');
  var label = name || 'Interaction';
  var total = (runningTotal !== undefined && runningTotal !== null) ? runningTotal : '';

  sheet.appendRow([label, formatted, total]);
}


function updateClientColumnL(clientName, text) {
  return updateInProgress(clientName, text);
}