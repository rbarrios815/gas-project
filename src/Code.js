...
// Assuming this is in a script tag or an external .js file linked to your HTML
function showNotesForSelectedClient() {
  var clientName = document.getElementById('clientDropdown').value;
  google.script.run.withSuccessHandler(displayClientNotes).getClientNotes(clientName);
}
...
function addAppointmentToCalendar(clientName, appointmentDate, appointmentTime) {
    ...
    if (event) {
        ...
        updateClientNote(clientName, followUpNote); // Assuming updateClientNote is a function to update the follow-up note
...
function updateClientChip(clientName, initials, dateInput) {
  ...
  ['RB','JB','TEAM','QC',''].forEach(function(k){ allowed[k] = true; });

  var initialsTrim = (initials || '').toUpperCase().trim();
  if (!allowed[initialsTrim]) {
    throw new Error('Initials must be RB, JB, TEAM, QC, or blank.');
  }
...
function updateInProgress(clientName, text) {
  var sh = SpreadsheetApp.getActive().getSheetByName('DASHBOARD 8.0');
  var last = sh.getLastRow();
  if (last < 2) throw new Error('No data rows.');

  // Normalize helper
  function normName(s) {
    return String(s || '')
      .replace(/\u00A0/g, ' ')      // NBSP -> space
      .replace(/\s+-\s+.*$/, '')    // drop " - Category" tail
      .replace(/\d+$/, '')          // drop trailing digits (if any)
      .trim()
      .toLowerCase();
  }

  var needle = normName(clientName);

  // Read only Column A (faster & simpler)
  var names = sh.getRange(2, 1, last - 1, 1).getValues(); // A2:A
  var foundRow = -1;

  for (var i = 0; i < names.length; i++) {
    var raw = names[i][0];
    if (!raw) continue;
    if (normName(raw) === needle) {
      foundRow = i + 2; // sheet row (offset by header)
      break;
    }
  }
  ...
  sh.getRange(foundRow, 12).setValue(text || ''); // Column L
